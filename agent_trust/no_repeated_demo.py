...
def process_submission_v2(character, game,  model="gpt-3.5-turbo-0613",  extra_prompt="", temperature=1.0, player_demographic=None):
    return get_res_for_visible_v2(role = character_info.get(character, ""), first_message= game_prompts.get(game, "No prompt available."), game_type= game, model_type =  model_dict[model],extra_prompt= extra_prompt, temperature=temperature, player_demographic= player_demographic)
...
1. You should select the persona for the trustor and the type of game.\n
